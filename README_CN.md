...
func main() {
    ...
    h.Use(cache.CacheByRequestURI(
        memoryStore,
        2*time.Second,
        cache.WithOnHitCache(func(c *app.RequestContext) {
            atomic.AddInt32(&cacheHitCount, 1)
        }),
        cache.WithOnMissCache(func(c *app.RequestContext) {
    ...
    h.GET("/get_hit_count", func(c context.Context, ctx *app.RequestContext) {
        ctx.String(200, fmt.Sprintf("total hit count: %d", cacheHitCount))
    })
    h.GET("/get_miss_count", func(c context.Context, ctx *app.RequestContext) {
        ctx.String(200, fmt.Sprintf("total miss count: %d", cacheMissCount))
    })
